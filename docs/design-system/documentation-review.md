...
Six reviewers worked from the source. None knew how the documentation was written or why.
Every finding was then checked against the running product. Each claim below has a file
and line behind it, or a measurement taken from a browser.
...
drifted. Everything `docs.js` reads out of the live stylesheet is correct, because it
cannot be anything else. That covers the token tables, the tier count, the large-text
scale and the keyframe catalogue. Almost every hand-typed number on the page is wrong:
the atom count, the spacing scale, the per-screen state counts, two deep links that go
nowhere. The root cause is structural. It is finding **P0-1**: no gate reads
`docs/design-system/`.
...
See **P0-10**, the finding that most undermines the page's own thesis.
...
- **Evidence:** `check.js` computes the README's headline state counts and fails the build
  ...
  `docs/design-system/`.
- **Why it matters:** This is the mechanism behind the numeric failures below. A gate
  derives the README's counts, so they cannot drift. The design system's counts, state
  links, atom totals and screens table are typed by hand and ungated. That is why they say
  `5/32/4/2`, "three screens", "Twelve", "three half-steps" and `#code/valid`. The parts
  `docs.js` generates from the live CSS are all correct. What a person typed is, with few
  exceptions, wrong.
- **Correction:** Extend `check.js` on the same principle it already applies to the
  ...
  match the class counts in `src/css`; that every `a-` / `m-` / `o-` / `t-` class
  defined in `src/css` appears at least once in the docs. Prose that cannot be computed
  stays prose. A number that can be computed should never be typed.
...
- **Evidence:** The docs say twice that error is not carried by colour alone because
  "the border thickens as well" (lines 500 and 692). It does not.
  `.a-field` sets `border: var(--size-border) solid …` (1.5px).
  `.a-field[data-state="error"]` changes only `border-color` and `background`.
  The one width change sits under `@media (forced-colors: active)`.
  A 2.5px error border exists only on `.a-otp-cell` (`atoms.css:374`).
- **Why wrong:** A WCAG 1.4.1 mitigation is documented for the flow's primary input but
  never implemented. The specimen renders right beside the claim, so catching it means
  measuring pixels.
- **Correction:** The non-colour carriers that *do* exist are the helper glyph and the
  shake (`foundations.css:47`). Either name those in both sentences or add
...
- **Why wrong:** Arithmetic fitted to a total is the one thing a traceability artefact
  must never contain.
- **Correction:** Use the measured figures. Say plainly that the columns overlap because
  these surfaces stack, so they are not a partition.
...
- **Source:** `src/css/organisms.css` has no `.o-outcome-panel {` base rule;
  `index.html:266` sets `class="o-signup-sheet__stage o-outcome-panel"`
- **Evidence:** The screens table gives it the template `t-device-frame`. The organisms
  section says an organism "positions itself absolutely inside the device frame".
  `.o-outcome-panel` has no base rule and never sets `position`. Compare
  ...
  `o-signup-sheet`.
- **Why wrong:** It states a containment relationship that does not exist. It is also why
...
- **Evidence:** The docs say "Each screen is a `<section>` labelled by its own heading;
  ...
  sheet is `role="dialog" aria-modal="true"`, which replaces the section role.
  ...
  `d-stateNav`, which is developer tooling.
- **Why wrong:** Three of four claims in one sentence are wrong. The shipping surface
  has no landmark structure at all.
- **Correction:** Describe what is actually there. Raise a separate production task to
  wrap the frame's product content in `<main>` in both files. **That is a production
...
- **Why wrong:** The first thing a reader learns about spacing is wrong. It cites as
  structural a value the system deliberately removed. A developer will hunt for a
...
- **Evidence:** `grep -in 'principle'` returns zero matches. `grep -in
  'contribut\|governance\|merge'` returns one match, which is prose about CSS defaults.
  The word "gate" appears seven times but never with a command, an owner or a merge
  condition.
- **Why wrong:** The page asserts throughout that rules are machine-enforced. It never
  tells a contributor how to run that enforcement or what blocks a merge, so the
  enforcement claim cannot be verified from the documentation itself. There is also no
...
- **Evidence:** measured by rendering the same classes in the docs page and in
  ...
  rule. Only two rules in the entire component library name a font family. Every
  specimen therefore inherits the *shell's* face and leading instead of the product's. The
  same cascade rule lets `.ds-main a:focus-visible` (unlayered, 2px `--ds-accent`)
  outrank the layered production ring (3px `--color-focus` plus halo) for every link
  inside a specimen. That sits directly under a caption reading "the ring is the real one,
  from the production CSS". `.ds-section p` also reaches into `.m-alert__title` and
...
- **Why wrong:** The page's thesis is that a specimen cannot drift because there is only
  ...
  All three come from the shell.
- **Correction:** Move the shell's type off `body` and onto `.ds-main` / `.ds-rail`. Then
  ...
  Scope the link ring to documentation chrome only. Change `.ds-section p` to a child
...
- **Note:** the earlier claim in this report's own scorecard that specimens are faithful
  ...
  was necessary but not sufficient. It missed inheritance entirely.
...
- **Evidence:** The specimen used `<span class="a-field">`. The `<label>` is what forwards
  ...
  tried first and was the wrong tool. The page attaches a Copy markup button that
  serialises the stage verbatim, so it was handing developers a component with its tap
  behaviour silently removed.
- **Correction:** Changed to `<label class="a-field">`. The prose now states that the
...
- **Evidence:** The caption reads "Every tone carries a glyph as well as a colour, so the
  ...
  it 3px under the title. `data-tone="info"`, the default and the tone the flow
...
\* `a-spinner` has since been added, so atoms now render 10 of 13. The gaps are named
...
- **Missing:** UI principles (2), Contribution and governance (14).
  ...
  **Misplaced:** `spacing` sits as a peer between primitives and atoms instead of inside
  Foundations and tokens. It is a token family, not a level of the UI hierarchy. Standing
  there it breaks the composition chain at exactly the point it starts.
...
- **Correction:** Reorder to the required sequence. Fold `architecture` and `naming` into
  Overview and Foundations respectively.
...
- **Evidence:** The only dark-surface specimen renders `.a-link--quiet` on `#0a1628`.
  `--color-text-secondary` is `#4C5973`, a light-surface ink. Measured on that ground it is
  **2.58:1**, below AA for body text. The correct variant, `.a-link--onDark`
...
- **Evidence:** The docs state an absolute prohibition. Production removes the ring in four
  places in `src/css` alone (and twelve more in the unmigrated page). Every one is
  defensible. The field and the OTP cell *substitute* a ring on their own edge. Both titles
  are `tabindex="-1"` headings focused by script that no keyboard user can reach.
- **Why wrong:** An absolute rule with no exception clause makes the codebase read as
  violating its own system in sixteen places. "One ring for the entire system" is not true.
...
- **Evidence:** Nine tokens carry measured ratios as comments. That includes the most
  safety-relevant fact in the scale: `--color-text-tertiary` is "3.72:1 on the sheet, so it
  is for inert controls only, never for a line being read". `docs.js` reads values via
  `getPropertyValue`, which discards comments, so none of it appears. The page says only
  "Contrast. Sampled from rendered pixels on all 43 states". It names not one pair.
- **Why wrong:** The restrictions are the half of each token's definition that prevents
  harm. They are written down in production then lost in the documentation.
- **Correction:** Add a contrast table: text token × surface token × measured ratio ×
  verdict. Mark the restricted tokens. Verified starting values:
...
- **Correction:** Add Level and Source columns (source from `styleSheet.href`). Render the
  `var()` target as a link to its own row. Harvest the usage restrictions from the CSS
...
- **Evidence:** The page says "in the product each one plays at its own duration token" and
  "Durations and easings are tokens, not numbers". Six declarations use raw numbers or bare
  CSS keywords, including three of the eight keyframes the catalogue renders.
...
- **Evidence:** CSS scales corner radii when the two radii on a side exceed the side
  ...
  (28), `--radius-phone` (47) and `--radius-pill` (999) all clamp to the same 13px stadium.
  `--radius-circle: 50%` draws an ellipse.
- **Why wrong:** The sample misinforms about the largest and most structural radii: the
  sheet corner, the phone corner and the pill are drawn identically.
...
- **Evidence:** Five distinct border widths ship: `1px` (four outline controls),
  ...
  `--size-hairline` 0.5px. The page never uses the phrase "border width". Both width tokens
  sit inside the generic Size family, drawn as sub-2px slivers.
- **Correction:** Add a Borders subsection: the two tokens, the `1px` outline-control
...
- **Evidence:** All 13 shadows render with complete composite values, which is correct.
  Ordering is missing. `tokens.css:281-283` carries the governing rules ("One light source,
  from above"; "Focus rings and glows are optics … not elevations"). Neither reaches the
  page.
...
- **Evidence, each verified:**
  - **Forced colours.** `@media (forced-colors: active)` exists in both files and is
    *enforced by `check.js`*, which fails the build without it. The design-system page
    mentions it nowhere. A developer adding a coloured state learns the rule only when the
    gate rejects them.
  - **Zoom and text resizing.** All 14 type steps are authored in `px` in both scales, so
    text-only resize does nothing. `[data-text="large"]` is the only resize path. It is a
    product setting, not a browser capability. Undocumented.
  - **Responsive spacing.** Seven real responsive spacing changes exist
    ...
    440px). `#responsive` says what varies is device width and the keyboard. It omits both
    breakpoints. No values are given anywhere.
...
- **Evidence:** Twelve organisms plus the persistent `o-tab-bar` return zero matches in the
  docs. 59% of the product's states and 76% of its screens have no name, no template
...
- **Evidence:** The docs state "A component does not impose its own external margins".
  Three components set a margin on their own root. `.m-sheet-header` already needs a cancel
  rule (`molecules.css:143`) to undo itself. The cost of the violation is in the file.
...
- **Evidence:** `.o-signup-sheet__stage[data-state="active"]` is a flex container. Five of
  its children are spaced by `margin-top`, deliberately, because `gap` cannot express "one
  step looser than the base": 8+8=16, 8+16=24.
- **Correction:** Restate as "gap owns the base rhythm; a child that must sit one or two
  ...
  flex children". Then list the seven.

### P1-16 · The only semantic spacing the system has is documented nowhere

- **Section:** `#spacing`
- **Source:** `organisms.css:245-267`
- **Evidence:** The organism carries a complete named rhythm: "every rendered gap is 8, 16
  or 24 and each number means one thing: 8 parts of one thing / 16 one group to the next /
  24 a decision, or a different route". In the docs it gets one bullet.
- **Why wrong:** The semantic spacing layer is documented as specified-but-unbuilt, which
  is honest and correct. But a *rendered* semantic rhythm already exists and lives only in
  a CSS comment. Anyone building a new stage will invent a fourth value.
- **Correction:** Add a "The three rendered gaps" subsection with the 8/16/24 table and the
  `gap + margin-top` composition rule.
...
- **Evidence:** Six consumers across three layers size themselves against it. It is
  ...
  `:root`. The only reason it resolves is that every consumer wrote a `0px` fallback.
- **Correction:** Declare `--keyboard-inset: 0px` in `tokens.css` so the fallbacks become
  belt-and-braces rather than load-bearing. Document it as a runtime-set token, naming its
  writer.

### P1-18 · Undocumented shipping components

- **Section:** `#atoms`, `#molecules`
- **Source:** `.a-scrim`, `.a-toast` and `.m-keyboard`. All three sit in `index.html`, the
  file the docs claim to cover completely
...
- **Section:** `#molecules`
- **Source:** `docs.js` `tidy()`, whose void-element list includes `path|circle|rect|use`
- **Evidence:** The HTML serialiser emits SVG children *with* end tags: `<rect …></rect>`,
  never `<rect …/>`. `tidy` treats the opening tag as void so it never indents, but the
  closing tag still dedents. Every SVG child removes a level it never added. In the
  `.m-privacy` specimen the second `m-privacy__row` prints at column 0 as though it were a
  sibling of `.m-privacy`. `</svg>`, `</span>` and `</div>` all close at the root.
  `.m-sheet-header` degrades the same way.
- **Why wrong:** Two of the five wired specimens print and copy markup whose indentation
  ...
  disagree. The tags are intact, so this is whitespace damage. It is also the exact text a
  developer pastes.
- **Correction:** Drop `path|circle|rect|use` from the void list. Make the indent symmetric
  per line: net opens minus closes, dedent before printing when negative.

### P1-20 · Two generated sections fail silently when nothing can be read

- **Section:** `#tokens`
- **Source:** `docs.js` `tokenTable()` and `tierTable()`
- **Evidence:** `tokenTable` builds an empty string when `TOKENS` is empty then assigns it,
  *erasing* the standing placeholder. The section headed "Every token" ends up containing
  nothing, with no message. `tierTable` in the same state renders "Of **0** tokens" with a
  large `0` in each cell and a note that explains only why the *second column* is
  uncountable. Both occur when `authoredTokens()` is blocked and `computedTokens()` returns
  nothing, which is any engine that does not enumerate custom properties on computed style.
- **Why wrong:** Every other reader on the page states its degraded mode out loud. These
  two are the exceptions. One of them renders a table of zeros that looks like a
  measurement.
...
- **The tier percentage is measurably wrong.** `docs.js` classifies a token by the tail of
  its name. It misfiles `--color-text-on-dark`, `--color-border-light` and three others as
  scale names and counts `--sp-1h` / `--sp-2h` as role names. The rendered "77% of the
  scale is named for its job" should be 80%. The page presents this as measured, so an
  error in the classifier is worse than an error in prose. *(Introduced by this session's
  work.)*
- **Forty of 191 tokens render no visual sample**, including the nine `--kb-*` colours. The
  swatch test matches only `--color-` and `--glass-`. Also unsampled: every `--z-`,
  `--tracking-`, `--dur-`, `--leading-`, `--ease-`, `--font-` and `--press-` token.
- **Roughly a fifth of colour swatches are invisible.** They are white and white-alpha
  tokens drawn on a white panel. Every on-dark token is in that group.
- **Two glass tokens draw a broken swatch**: `--glass-sheet-bd` and `--glass-panel-bd` hold
  `blur(…) saturate(…)` filters. `sampleFor` emits them as a `background` value.
- **Elevation swatches clip.** `--shadow-phone` is `0 60px 200px` on a 30px chip inside an
  `overflow-x: auto` wrapper. The largest elevations are the ones the sample cannot show.
...
- **The molecule count is delivered against by a third.** Eleven blocks exist, which is
  correct, but only four have specimens.
- **Opacity has no scale and six competing raw values** (`.85 .55 .5 .45 .35 0.20`). Three
  of them are different answers to "this control is inert". Undocumented entirely.
- **Traceability is missing two links of its own chain.** The stated chain is screen →
  template → organism → molecule → atom → tokens. The table has no template column and no
  source-file column. The Privacy row's atom cell is `—`.
...
- **A second, hand-typed component spec exists.** `index.html` carries a `RECIPES` array of
  33 entries covering the same components this page documents. It has already drifted. It
  records the OTP cell's font as `22px` where the CSS uses `var(--text-title)`, 24px under
  large text.
- **Twenty one-off spacing values** sit in the component layers with no approved-exceptions
  list. They mix considered optical nudges (`margin-top: -1px /* optical, against cap
  height */`) with unexplained values (`.a-toast { bottom: 96px }`).
- **`#privacyCard:not([hidden])` is an ID-scoped layout rule in the foundations layer.** It
  is the only ID selector in the product CSS. It sits outside the documented ownership
  model.
...
- **The templates section makes three wrong claims in one sentence.** The sheet does not
  ...
  Android notch is a 14px hole-punch rather than a pill. The title step-down is triggered by
  sheet presence, not by the keyboard.
...
- **Nothing links to the documentation.** `grep -c "design-system"` returns 0 for both
  `README.md` and `index.html`. The page is committed and deployed. A reviewer following the
  README reaches the prototype and never learns it exists.
- **The generated family headings are styled by the browser, not the stylesheet.**
  ...
  the section. "Colour", "Spacing", "Duration" fall back to UA `<h3>` styling and visibly
  differ from every hand-written `h3` above them.
- **`docs.css` hand-copies four production colour values**: `#0a1628` (`--color-hero-1`),
  `#e8f5ee`, `#166832`, `#1650b8`. Its own header comment says the shell's scale is
...
- **Two movements leave the specimen invisible.** `sheetDown` and `scrimOut` end at
  `opacity: 0` and play with `fill-mode: both`. Pressing Play empties the stage permanently
  with no reset.
- **The motion catalogue is dead under OS reduced-motion** and says nothing about why. The
  page explains twice that the *product* honours reduced motion so the flow stays
...
- **The 18 `--d-*` tokens are filtered out of "Every token" silently.** They are declared in
  `tokens.css`, not `developer.css`, so the page loads them and drops them. The architecture
  table implies they are not present at all.
- **The tier section's live count carries a hand-written enumeration** naming the gradients,
  ...
  tokens and are not in the list. That is a second source of truth inside the one mechanism
...
- **The documentation page has no `<h1>`.** Its name is carried by a `<p>`. It does have
  `<main>` and a labelled `<nav>`, so this is the one structural defect in an otherwise
  well-formed document.
- **The README understates the tap-target floor.** It says 44px. `--size-target` is 48px and
  `sweep.js` reads the token.
- **`rem` values are truncated** at four decimals, so `0.5px` renders `0.0313rem`. The rem
  column also fires on `--tracking-*` where it is meaningless.
...
- **`--font-mono` is developer-only** but is listed beside the product face with no
  distinction. `--kb-*` is correctly annotated.
- **The large-text specimen caps its glyphs at 28px**, so `--text-figure`'s 64→68 step is
  drawn as two identical characters. The explanation is only in the source.
...
- **"The scale is copied verbatim into both pages" is out of date.** `index.html` now links
  `tokens.css` and holds no declarations of its own.
- **`sampleFor` interpolates a token value into an HTML attribute without `esc()`.** It is
  the one path in the file that skips the escaper used everywhere else. Nothing breaks today
...
- **The rail highlight has no fallback** without IntersectionObserver. It marks nothing on
  first paint. Below 900px the stacked layout scrolls it off-screen entirely.
...
- **The single-tier analysis is the strongest writing on the page.** It does not overclaim a
  reference tier or understate the semantic naming. Its warning against a half-done palette
  is the right warning. Verified: 208 declarations, all in `tokens.css`, none
  component-scoped.
...
- **The `--size-target` floor is real and honoured.** 48px, read from the token by the sweep
  tool. The sheet's close control extends a 36px circle to 48 via a pseudo-element.
...
completely and correctly. They cannot learn the type system, the borders, the transitions,
the design principles or how to contribute, all absent. 76% of the product's screens are
...
terminates before reaching the atom level. The app's 13 screens are compressed into a single
row. The sign-up screens table contains counts that match no real grouping.
...
component rule. But it leaks by inheritance. Every specimen renders in the shell's typeface
and leading rather than the product's. Links inside a specimen get the shell's focus ring
instead of the system's (P0-10, 17 measured drifting properties). Two further defects are in
the specimens themselves: one chooses the wrong variant for its ground (P1-2) and one caption
names a variant the stage does not contain. The architecture is right. The isolation is
incomplete.
...
actually renders is documented only in a CSS comment. No responsive spacing value appears
anywhere.

**5. Does the documentation accurately apply Brad Frost's hierarchy?**
Partly. Tokens are correctly kept out of the atomic levels and are never called atoms, so
the critical conflation test passes. Pages and screens are correctly treated as one level.
But `spacing` stands as a peer between primitives and atoms. `o-outcome-panel` is documented
at the wrong level. The `p-` page level exists in the naming table with no layer to live in.
The required section order is not followed.
...
- **P0-2**. Whether the error field should thicken its border, or the docs should describe
  the glyph and shake instead.
...
*Structural, the highest-value change of all:*
- **P0-1**. Gate the documentation. Until a tool derives these numbers they will drift
  again. The next review will find the same class of error in different places.

Two defects in this report were introduced by the documentation work itself and are marked
as such (P1-2 and the tier-percentage classifier). They are recorded at the same severity as
everything else.
...
and `.ds-main`. `.ds-spec__stage` now re-asserts the product's inherited context. The link
focus ring is scoped to documentation chrome, so it can no longer repaint a specimen.
...
Re-measured, like with like: `.a-btn--primary` and `.m-alert__title` are now
**computed-identical** to the same classes in `index.html`. Drift fell from 17 properties to
2. Both survivors are `min-height: auto` versus `0px`, a flex-versus-block context
...
**False statements removed.** The error field no longer claims a thickening border. The real
non-colour cues are named instead: the helper glyph, the shake and the forced-colours dashed
outline. Forced colours is now a documented accessibility dimension (P0-2 documentation
half, P1-11). The two dead deep links point at real states (P0-3). The spacing scale says
two half-steps and no 18, corrected in the docs and in the CSS comment in both files it was
copied from (P0-7). The atom and molecule counts are accurate. The gaps are named rather
than concealed (P1). The `developer` layer row no longer claims never to ship. The five
tooling rules in `templates.css` are disclosed (P1-13 documentation half). Two specimen
captions no longer advertise variants their stages do not contain.

**The resend entry was itself wrong and is rewritten (P2).** It claimed two rules on the
atom and two on the organism. It also claimed layers would have flipped them. Measured:
three of the four are in `atoms.css`. Both competing weight rules sit inside `@layer atoms`,
so layer order cannot separate them. Source order alone decides. The entry now describes the
real problem. Pinning to preserve the pixels left three organism rules filed under atoms,
and fixing the filing would change the rendering. That is a design decision, not a refactor.

**README.** Now links the design system. It states the tap-target floor as 48px from
...
`aria-invalid` did not, on two of the five paths that turn the field red: the wrong-length
number and the landline. A field went red and a screen reader was told nothing was wrong.

Fixed at the cause rather than at the two sites. The look and the announcement are now one
...
Both pages had none. The only `<nav>` was the developer state rail. The stage element that
holds the phone is now `<main>` rather than `<div>`, a tag swap rather than a new element.
Both are block-level, so nothing moves. Its selector was already class-based. Verified by
the full sweep: 104 states, no change.
...
of equal specificity happened to be typed after it. Removing it changes no pixels and kills
the contradiction. The ruling is now what the stylesheet says rather than what it
accidentally does.

### The gate now reads the documentation (P0-1)

`check.js` gained a section. It is additive: no existing assertion was touched or weakened.
Four new checks:
...
2. Every block defined in `src/css` is mentioned on the page, so a component cannot ship
   undocumented.
3. Every block the page names exists, either in `src/css` or in the unmigrated page, whose
   own stylesheet is the allowlist rather than a hand-written list that would go stale.
...
(`a-filter-chips`, where production has `a-filter-chip`). It also rejects a fabricated class
in a negative test.
...
into Foundations and tokens, because spacing is a token family rather than a level of the UI
...
- **Nine consolidation candidates** are listed as proposals with their risks. None is
  applied. Production typography does not change without a ruling. Several would move pixels.
...
- **The focus-ring count.** The integrity probe focused each control with `el.focus()` and
  ...
  which is a keyboard affordance. So the check was measuring nothing. Its earlier
  "0 without a ring" was luck rather than evidence. Rebuilt to press real Tab keys: 130
  controls, 0 without a ring. Prose links take the documentation's 2px ring. Scroll
...
- **Journey failures that were not real.** Six, then nine, then eight failures appeared, each
  ...
  measure before diagnosing, then check whether the failing check is itself wrong.
...
rotation). On the app page it is **50** (entrance reveals, a pulsing live dot, a medal wave)
across 17 element types and exactly two properties, transform and opacity.
...
nothing uses. It is right to: a semantic name with no consumer is decoration. Eight
...
It declares `@layer templates` rather than `@layer developer`. That is the whole
...
already outranks them. That is a cascade change wearing a tidy-up's clothes. The file is
linked immediately after `templates.css`, so within that layer the rules sit exactly where
they sat. **They changed address, not rank**, which is why the fingerprint is unchanged.
...
carried its own fallback. Those fallbacks were load-bearing rather than defensive, so
...
`.o-signup-sheet__resend` rules are now in `organisms.css`. The fingerprint is
unchanged at both text sizes. **The move stands and no exception was required.** The filing
law holds again. The countdown's 500 / ready 600 split is now what the stylesheet says
rather than what source order happened to produce.

### The gate learned two more families

`check.js` gained `--space-` and `--keyboard-` as sanctioned token families. Its docs
...
between the two files. The token count had moved from 205 to 213. Neither would have been
noticed by eye.
...
properly, which means over http. Off disk a browser will not let a page read a linked
stylesheet, so the unused-token check had been reporting *"stylesheet rules are unreadable
here, so this could not be checked"*. Honest. It is also the reason nobody had seen what it says
...
and all four were equally unused at HEAD. Pre-existing, invisible because the only way to
see it was a run nobody was doing.

They are now named in `SPENT_BY_THE_OTHER_FLOW` with what spends them. That is the
mechanism the list exists for: named rather than pattern-matched, so the day one of them
stops being used on *both* sides it comes back as a failure. The check now reads
**10 passing, 1 warn**. The warn is the demo support number, which is long-standing.
